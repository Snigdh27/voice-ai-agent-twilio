...
if not MUSIC_FILE.exists():
    raise RuntimeError(
        f"Music filler not found: {MUSIC_FILE}"
    )


MUSIC_BYTES = (
    MUSIC_FILE.read_bytes()
)
...
print(
    "GENIE_IDP_USER_ID fallback:",
    (
        "configured"
        if os.getenv("GENIE_IDP_USER_ID")
        else "MISSING"
    ),
)
print(
...
def get_call_state(
    ...
    if call_sid not in call_states:
        ...
        call_states[
            ...
            # Dynamic Workato identity for this call.
            # None means genie.py will use .env fallback.
            "workato_x_idp_user_id":
                None,
...
# OUTBOUND:
#   https://host/twiml
#       ?workato_x_idp_user_id=<dynamic-id>
#
# ============================================================
...
async def twiml(
    ...
    ws_url = (
        f"wss://{PUBLIC_HOST}/media-stream"
    )


    safe_idp_user_id = escape(
        dynamic_idp_user_id
        or ""
    )
...
def conversation_is_quiet(
    state,
    quiet_for=
        SILENCE_BEFORE_MUSIC,
):
...
async def backend_music_fillers(
    *,
    twilio_ws,
    stream_sid,
    state,
    ...
    try:
        ...
        while not done_event.is_set():
            ...
            if conversation_is_quiet(
                state
            ):

                print(
                    "Silence detected - playing filler"
                )


                await play_music_filler(
                    twilio_ws=
                        twilio_ws,
    ...
    *,
    ...
    try:
        ...
        await append_thinking(
            openai_ws,
            (
                "Workato Genie has completed the "
                "delegated business request. "

                "Use the supplied result as the "
                "source of truth. "

                "Do not invent business facts. "

                "Communicate it naturally "
                "and concisely."
            ),
            delegation_id=
                delegation_id,
                ...
                openai_ws,
                {
                    ...
                    "session": {
                        ...
                        "instructions": (
                            ...
                            "You are a concise, natural "
                            "phone assistant powered by "
                            "Workato Genie. "

                            "Keep the conversation responsive "
                            "and natural. "

                            "Before delegating a business "
                            "request, collect obvious required "
                            "identifiers from the caller. "

                            "For order status or cancellation, "
                            "if the caller has not provided an "
                            "order number, ask for it directly. "

                            "Do not delegate merely to discover "
                            "that an obvious identifier is "
                            "missing. "

                            "When enough information is "
                            "available, delegate the business "
                            "request to the client application. "

                            "When you delegate, immediately "
                            "acknowledge the caller with one "
                            "short sentence such as "
                            "'Thanks, I'll check that now.' "

                            "The backend may continue working "
                            "after you acknowledge the caller. "

                            "Remain available to listen while "
                            "the backend is running. "

                            "When backend commentary arrives, "
                            "communicate the useful result "
                            "naturally and concisely. "

                            "Preserve important business facts "
                            "returned by Workato Genie. "
                        ...
                        async def prewarm():
                            ...
                            try:
                                ...
                                genie_id = (
                                    ...
                                    await ensure_genie_conversation(
                                        state
                                    )
                                )
